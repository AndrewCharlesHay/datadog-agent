...
static __always_inline void handle_congestion_stats(conn_tuple_t *t, struct sock *sk) {
    ...
    // CO-RE: use bpf_core_field_exists to guard reads of fields that don't
    // exist on all kernels. The CO-RE loader + verifier correctly prune the
    // dead branch when the field is missing.
    // Runtime: use LINUX_VERSION_CODE guards for the same purpose.
    ...
    // Bit 0 (TCP_ECN_OK) indicates ECN was successfully negotiated.
    {
        __u8 ecn = 0;
