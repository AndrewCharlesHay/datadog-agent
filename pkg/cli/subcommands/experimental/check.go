...
// Returns (true, nil) if permissions look good, or (false, error) with
// context if they don't. Only call on non-Windows systems; Windows uses
// ACLs rather than Unix-style mode bits so the check is not applicable.
...
func validateAPIKey(apiKey string) (message string, err error) {
	...
	masked := scrubber.HideKeyExceptLastFiveChars(apiKey)
...
func runConfigCheck(_ log.Component, cfg config.Component, noAPICheck bool) error {
	...
	// 1. File permissions — skipped on Windows (ACL-based, Unix mode bits do not apply)
