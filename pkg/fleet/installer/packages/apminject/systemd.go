...
func (s *SystemdServiceManager) Install(ctx context.Context) (err error) {
	...
	scriptInstaller := &InjectorInstaller{}
	if scriptErr := scriptInstaller.addInstrumentScripts(ctx); scriptErr != nil {
		scriptInstaller.Finish(scriptErr)
		return fmt.Errorf("failed to install instrument scripts: %w", scriptErr)
	}
	scriptInstaller.Finish(nil)
...
func (s *SystemdServiceManager) Uninstall(ctx context.Context) (err error) {
	...
	if err := (&InjectorInstaller{}).removeInstrumentScripts(ctx); err != nil {
		log.Warnf("Failed to remove instrument scripts: %v", err)
	}
