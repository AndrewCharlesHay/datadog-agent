...
func newGRPCClient(endpoint config.Endpoint, cfg pkgconfigmodel.Reader) (*grpc.ClientConn, statefulpb.StatefulLogsServiceClient, error) {
	...
	opts = append(opts, grpc.WithUserAgent(fmt.Sprintf("datadog-agent/%s", version.AgentVersion)))
	opts = append(opts, grpc.WithPerRPCCredentials(&headerCredentials{endpoint: endpoint}))
